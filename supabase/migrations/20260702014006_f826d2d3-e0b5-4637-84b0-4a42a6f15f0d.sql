...
BEGIN
  ...
  -- Recent momentum: net votes in the last 30 days, and their share of lifetime activity.
  ...
  v_recent_ratio := CASE WHEN ABS(v_total_net) > 0
                         THEN v_recent_net / GREATEST(ABS(v_total_net), 1)
                         ELSE 0 END;
  ...
  -- Momentum in [-1, 1]: primarily the sign & magnitude of recent votes, boosted by lifetime lean.
  v_momentum := GREATEST(-1.0, LEAST(1.0,
      TANH(v_recent_net / 40.0) * 0.75
    + TANH(v_total_net  / 250.0) * 0.20
    + (v_pop_rank - 0.5) * 0.10
    + v_recent_ratio * 0.20
  ));

  -- Drift is now momentum-driven. Bullish votes push up; bearish votes push down.
  v_drift := v_momentum * 0.028
           + (v_pop_rank - 0.5) * 0.006;

  -- Per-term volatility: quieter floor, wider ceiling.
  -- Low-popularity/low-activity terms: ~0.03. Viral/spiky terms: ~0.55.
  v_vol := 0.03
         + LEAST(0.22, COALESCE(v_peak, 40) / 100.0 * 0.28)          -- peak height
         + LEAST(0.18, COALESCE(v_pop_stddev, 0) / 100.0 * 0.50)     -- trajectory choppiness
         + LEAST(0.10, GREATEST(0, v_pop_rank - 0.35) * 0.22)        -- popularity boost
         + LEAST(0.08, ABS(v_total_net) / 300.0 * 0.10)              -- lifetime activity
         + LEAST(0.06, ABS(v_recent_net) / 80.0 * 0.10);             -- recent activity spike
  ...
  steps AS (
    SELECT i.month_ts, i.i, c.cum_net, i.prev_v, i.next_v, i.prev_age, i.next_age,
      (
        v_drift
        -- Seasonal cycles: biased slightly in the direction of momentum (bullish = more upside sine).
        ...
        -- Momentum-skewed noise: same volatility magnitude, but center shifts toward the momentum sign.
        + v_vol * 0.070 * (
        ...
        -- Occasional shock: more frequent and larger for viral terms; direction biased by momentum.
        + CASE
