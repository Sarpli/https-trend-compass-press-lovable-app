...
BEGIN
  ...
  -- Build organic stock-like noise. We mix three sine waves at incommensurate
  -- frequencies (so the pattern never visibly repeats) with TWO independent
  -- pseudo-random jitter streams and a sparse "shock" stream that occasionally
  -- spikes a single month up or down — like a viral moment or a backlash.
  noisy AS (
    SELECT s.month_ts, s.intensity, s.cum_net,
      s.intensity * (
        ...
        -- Primary jitter: every month gets a unique random nudge.
        + v_vol * 0.40 * ((('x' || substr(md5(_trend_id::text || ':a:' || s.i::text), 1, 8))::bit(32)::bigint % 2000 - 1000) / 1000.0)
        -- Secondary jitter with a different salt so consecutive months don't
        -- look like a smooth staircase.
        + v_vol * 0.25 * ((('x' || substr(md5(_trend_id::text || ':b:' || s.i::text), 1, 8))::bit(32)::bigint % 2000 - 1000) / 1000.0)
        -- Sparse shock: ~1 in 7 months gets an extra ±18% spike.
        + CASE
